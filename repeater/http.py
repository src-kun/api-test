...
class HttpConnectionException(HttpBaseException):
	pass

class HttpDataException(HttpBaseException):
	pass

class HttpNoneDataException(HttpBaseException):
	pass
...
class Request():
	...
	def __accept(self, url):
		if url is None:
			errMsg = "url is None !"
			logger.error(errMsg)
			raise HttpNoneDataException(errMsg)
		elif cmp(url[0:4], "http"):
			errMsg = "{" +url + "}" + " You must start with (http[s]://)"
			logger.error(errMsg)
			raise HttpDataException(errMsg)
